...
- `mode` — `highlight`, `isolate`, `changed`, or `notes` (view mode; UI labels: `isolate` = **Only highlighted**, `notes` = **Only notes**)
...
#### Scenario: Mode only notes

- **WHEN** the URL contains `mode=notes`
- **THEN** view mode is Only notes
