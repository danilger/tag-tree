...
- **Hide** — neither file-level comment nor line notes are shown
...
#### Scenario: Operator switches to Hide

- **WHEN** the operator selects Hide and the node has a file `comment` and/or notes
- **THEN** the file summary block is hidden and no inline note annotations are shown
